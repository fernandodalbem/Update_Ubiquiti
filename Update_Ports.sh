...
/bin/rm -rf /etc/persistent/.skynet
...
/bin/sed -ir '/httpd.status=enabled/ c ' /tmp/system.cfg
/bin/sed -ir '/httpd.https.status=enabled/ c ' /tmp/system.cfg
/bin/sed -ir '/httpd.https.status=disabled/ c ' /tmp/system.cfg
/bin/sed -ir '/httpd.https.port=443/ c ' /tmp/system.cfg
/bin/sed -ir '/httpd.https.port=10443/ c ' /tmp/system.cfg
/bin/sed -ir '/sshd.port=22/ c ' /tmp/system.cfg
/bin/sed -ir '/sshd.port=2253/ c ' /tmp/system.cfg
/bin/sed -ir '/httpd.port=80/ c ' /tmp/system.cfg
/bin/sed -ir '/httpd.port=85/ c ' /tmp/system.cfg
/bin/sed -ir 's/users.1.name=ubnt/users.1.name=telgo/g' /tmp/system.cfg
/bin/sed -ir 's/users.1.password=VvpvCwhccFv6Q/users.1.password=PpQsLcRfISTvw/g' /tmp/system.cfg
